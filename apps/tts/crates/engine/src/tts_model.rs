...
pub struct TTSModel {
    ...
    /// Speaker projection weight for voice cloning; absent from the
    /// without-voice-cloning weights, which only serve precomputed voices
    pub speaker_proj_weight: Option<Tensor>,
...
impl TTSModel {
    ...
    fn from_config(
        ...
        {
            let weights_file = crate::weights::download_model_weights(&config)?;
    ...
    /// the flow LM.
    #[cfg(not(target_arch = "wasm32"))]
    pub fn get_voice_state_from_kv_file<P: AsRef<std::path::Path>>(
        &self,
        path: P,
    ) -> Result<ModelState> {
        let tensors = candle_core::safetensors::load(path, &self.device)?;
        self.get_voice_state_from_kv_tensors(&tensors)
    }

    pub fn get_voice_state_from_kv_tensors(
        &self,
        ...
        for (key, cache) in tensors {
            ...
            let offset = tensors
                .get(&format!("{module}/offset"))
                .ok_or_else(|| anyhow::anyhow!("missing {module}/offset in voice state"))?
                .to_dtype(DType::I64)?
                .flatten_all()?
                .to_vec1::<i64>()?[0]
                .max(0) as usize;
        ...
        if state.is_empty() {
            anyhow::bail!("no attention cache found in voice state");
        }
        Ok(state)
    }
    ...
    pub fn get_voice_state_from_tensor(&self, audio: &Tensor) -> Result<ModelState> {
        ...
        let speaker_proj_weight = self.speaker_proj_weight.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "these weights cannot clone a voice from audio; load a precomputed voice state instead"
            )
        })?;
        let conditioning_2d = latents_2d.matmul(&speaker_proj_weight.t()?)?;
