...
use crate::modules::attention::{StackedKv, StreamingMultiheadAttention};
...
impl StreamingTransformer {
    ...
    /// `x` is [B, 1, D]; row `i` attends against row `i` of every cache.
    pub fn forward_stacked(&self, x: &Tensor, caches: &mut [StackedKv]) -> Result<Tensor> {
        let mut x = x.clone();
    ...
    pub fn layer_state_name(&self, i: usize) -> String {
        format!("{}.layers.{}.self_attn", self.name, i)
    }
}
