...
/// The voice-cloning weights live in a gated repository; without access to
/// them, the same model minus the speaker encoder serves precomputed voices.
#[cfg(not(target_arch = "wasm32"))]
pub fn download_model_weights(config: &crate::config::Config) -> Result<PathBuf> {
    let primary = config.weights_path.as_deref();
    let fallback = config.weights_path_without_voice_cloning.as_deref();
    let first_error = match primary {
        Some(path) => match download_if_necessary(path) {
            Ok(file) => return Ok(file),
            Err(e) => e,
        },
        None => anyhow::anyhow!("weights_path not specified in config"),
    };
    match fallback {
        Some(path) => {
            tracing::warn!(error = %first_error, "voice-cloning weights unavailable, loading the without-voice-cloning weights");
