...
impl SimpleMLPAdaLN {
    ...
    pub fn precompute_modulations(
        ...
        // c_emb: [B, 512], time_embeddings: [S, 512]; every (row, step)
        // modulation comes out of one matmul per block.
        let (batch, cond) = c_emb.dims2()?;
        let num_steps = time_embeddings.dim(0)?;
        let y = time_embeddings
            .unsqueeze(0)?
            .broadcast_add(&c_emb.unsqueeze(1)?)?; // [B, S, 512]
        let y_silu = y.silu()?.reshape((batch * num_steps, cond))?;
        let per_step = |mod_batch: Tensor, s: usize| -> Result<Tensor> {
            let width = mod_batch.dim(1)?;
            mod_batch
                .reshape((batch, num_steps, width))?
                .narrow(1, s, 1)?
                .squeeze(1)
        };
        ...
        for block in &self.res_blocks {
            ...
            for s in 0..num_steps {
                let modulation = per_step(mod_batch.clone(), s)?; // [B, 1536]
